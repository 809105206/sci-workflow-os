...
3. If no active project exists and the user supplied a research direction, create `workspace/<short-slug>` with `sciops init`, activate it, record the direction in `research-state.yaml`, and continue G0.
4. If several project candidates exist and the request does not identify one, ask the user to select it.
...
- G0-G1: convert direction into a bounded project charter, candidate topics, feasibility constraints, and stop rules.
...
- G7: draft a complete evidence-bound manuscript with declarative prose and traceable figures, tables, numbers, and citations.
...
1. Apply `standardize-academic-writing` when drafting or revising scholarly text.
2. Run `sciops writing lint <manuscript> --strict` before declaring a draft ready.
3. After each meaningful milestone, run `sciops codex checkpoint` with the completed action and ordered next actions.
4. Leave the active project with one concrete first next action so a new Codex session can resume immediately.
